...
export async function ddl() {
  ...
  const params = {
    ...
    datasetName: dbName,
    workStatus: 2,
    hosts: 'https://xxx.com', // 可空
    manageAddresses: 'glitter1rjj69vzkuqc7jqtlggj8dqd30ar2myemur2p6j',
    meta: 'datasetmeta',
    description: '{"github":"xxx3.com"}', // object json string
    duration: Long.fromNumber(86400), // seconds
  };
  ...
  const meta = `"CREATE TABLE testcreatedataset.${bookTableName} (    id INT(11) PRIMARY KEY COMMENT 'id',     name VARCHAR(255) COMMENT 'name of book',    category VARCHAR(255) COMMENT 'book category (eg:history,cartoon..)',    author VARCHAR(255) COMMENT 'author',    public_at INT(11) COMMENT 'public time',    stock INT(11) COMMENT 'stock of book',    price INT(11) COMMENT 'price',    _tx_id VARCHAR(255),    KEY 'name_ind' (name),    KEY 'name_author_ind' (name, author)) ENGINE=standard;"`;
  ...
  const releaseRst = await dbClient.pledge(releasePledgeParams);
