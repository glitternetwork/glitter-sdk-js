...
export class MnemonicKey extends RawKey {
   ...
   *
   * ```ts
   * import { MnemonicKey } from 'glitter.js';
   *
  ...
  constructor(options: MnemonicKeyOptions = {}) {
    ...
    const masterKey = HDNode.fromMnemonic(mnemonic);
    const hdPath = `m/44'/${coinType}'/${account}'/0/${index}`;
    const glitterHD = masterKey.derivePath(hdPath);
    const privateKey = glitterHD.privateKey;
    ...
    super(Buffer.from(privateKeyWithoutPrefix, 'hex'));
