...
const glitter = new LCDClient({
  chainID: 'xian',
  // URL: 'https://pisco-lcd.glitter.dev',
  URL: 'https://api.xian.glitter.link"',
});

const glitterClassic = new LCDClient({
  chainID: 'xian',
  // URL: 'https://lcd.glitter.dev',
  URL: 'https://api.xian.glitter.link"',
});

const txAPI = new TxAPI(glitter);
const db = glitter.db(mk);

describe('TxAPI', () => {
  describe('decode', () => {
    it('classic', async () => {
      glitterClassic.tx.decode(
  ...
  describe('broadcast', () => {
    ...
    it('broadcast fetches and returns txInfo', async () => {
      ...
      const send = new MsgSend(
        'glitter18xaq07t83yvseqqt3urmqus9qlka4kefa4kyfj',
        'glitter18xaq07t83yvseqqt3urmqus9qlka4kefa4kyfj',
        { uluna: '1000000' }
      );

      const tx = await db.createAndSignTx({ msgs: [send] });
